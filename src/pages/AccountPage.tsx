...
export default function AccountPage() {
  ...
  return <main className="store-shell account-page">
    ...
    {loading ? <div className="prototype-note wide-note"><LoaderCircle size={18} className="spin" /> Loading software...</div> : error ? <div className="checkout-error">{error}</div> : software.length === 0 ? <div className="prototype-note wide-note">No software licenses are attached to this account yet.</div> : software.map(item => <article className="library-card" key={item.entitlementId}><div className="library-product"><img src={streamSafe.icon} alt="" /><div><h2>{item.productName}</h2><span>{item.environment === "test" ? "Test Purchase" : "Lifetime License"}</span></div><span className="status-pill">{item.environment === "test" ? "TEST" : item.status}</span></div><div className="library-details"><div><span>License key</span><div className="library-license"><code>{item.licenseKey}</code><button onClick={() => void copyKey(item.licenseKey)}><Copy size={15} /></button></div></div><div><span>Version</span><strong>v{item.version}</strong></div><div><span>Platform</span><strong>Windows</strong></div></div><div className="library-actions"><button className="button primary" type="button" disabled={!item.canDownload} onClick={() => void download(item)}><Download size={16} /> {item.canDownload ? "Download Latest" : "Download Disabled"}</button><a className="button secondary" href="/support"><FileText size={16} /> Installation Guide</a></div></article>)}
  </main>;
}
